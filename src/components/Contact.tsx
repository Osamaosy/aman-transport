      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-emerald-100 max-w-3xl mx-auto">
            نحن هنا لخدمتك على مدار الساعة. تواصل معنا للحصول على استشارة مجانية
            وعرض سعر مخصص
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-6 mb-16">
          {contactInfo.map((info, index) => (
            <div
              key={index}
              className={`bg-white p-6 rounded-2xl text-center shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 ${
              ...
              <div className="space-y-2">
                {info.details.map((detail, detailIndex) => (
                  <div key={detailIndex}>
                    {info.type === "phone" ? (
                      <a
                        id={`call_button_${index}`}
                        href={`tel:${detail}`}
                        onClick={() => {
                          // تصحيح: إرسال الـ ID فقط (حذفنا الرابط من هنا)
          ...
          <div className="bg-white/10 backdrop-blur-sm p-8 rounded-3xl shadow-2xl max-w-4xl mx-auto border border-white/20">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <a
                ...
                onClick={() => {
                  // تصحيح: إرسال الـ ID فقط (حذفنا الرابط من هنا أيضاً)
